...
@Service("servingop")
public class MLServingOperation implements Function<WorkflowConfig, Boolean> {

    private static final Logger LOG = LoggerFactory.getLogger(MLServingOperation.class);

    public Boolean apply(WorkflowConfig config) {
        try {
            WorkflowSpec p = new MLWorkflow().createServingPipeline(config);
